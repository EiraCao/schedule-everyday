...
【分类方式 D】选择以上任意分类方式，再加上 Exploration 类别，以 MoSCoW + Exploration 为例：
- [事项1] → Must（必须做）
- [事项2] → Should（应该做）
- [事项3] → Could（可以做）
- [事项4] → Someday（无优先级）
...
特点：引入了“无优先级”的维度；Exploration 是不承诺任何产出的事项，当承诺了产出就可以随时加入到其它类别中，且优先级可高可低。**主要是针对想保护自己的灵感、又对过长的“待办事项”有压力的人**。
...
【分类方式 D】，以 MoSCoW + Exploration 为例：
- [事项1] → Must（必须做）
- [事项2] → Should（应该做）
- [事项3] → Could（可以做）
...
**日历写入方式**：
- 方式一（推荐）：OpenClaw 在自己日历上创建，用户订阅 OpenClaw 日历
- 方式二：用户授权写入ta自己的日历，这比较麻烦，需要说清楚怎么做
